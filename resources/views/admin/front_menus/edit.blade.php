<x-admin-layout>
    ...
    <x-slot name="content">
        <x-admin.container-card>
            ...
            <form method="POST" action="{{ route('manage.frontmenus.edit', $frontmenu->id) }}" id="quickForm">
                @csrf
                @php
                    $type = '';
                @endphp
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="select2-purple">
                                    <input type="hidden" name="fk_menu_type_id"
                                        value="{{ $frontmenu->fk_menu_type_id }}" />
                                    <select class="form-control" disabled>
                                        @if ($front_menu_types)
                                            @foreach ($front_menu_types as $id => $title)
                                                ...
                                                @endphp
                                                <option value="{{ $id }}"
                                                    {{ old('fk_menu_type_id', $frontmenu->fk_menu_type_id) == $id ? 'selected' : '' }}>
                                                    {{ $title }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <div class="form-group" id="page_module">
                                @if (old('menu_type', $frontmenu->menu_type))
                                    @if (old('menu_type', $frontmenu->menu_type) == 1)
                                            ...
                                            <select name="fk_page_id" class="form-control">
                                                ...
                                                @if ($front_menu_types)
                                                    @foreach ($pages as $id => $slug)
                                                        <option value="{{ $id }}"
                                                            {{ old('fk_page_id', $frontmenu->fk_page_id) == $id ? 'selected' : '' }}>
                                                            {{ $slug }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                    @elseif(old('menu_type', $frontmenu->menu_type) == 2)
                                            ...
                                            <select name="fk_page_id" class="form-control">
                                                ...
                                                @if ($access_list)
                                                    @foreach ($access_list as $id => $slug)
                                                        <option value="{{ $id }}"
                                                            {{ old('fk_page_id', $frontmenu->fk_controller_route_id) == $id ? 'selected' : '' }}>
                                                            {{ $slug }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                    @elseif(old('menu_type', $frontmenu->menu_type) == 3)
                                        <x-label for="fk_page_id">Custom URL <span
                                                class="text-danger">*</span></x-label>
                                        <div class="select2-purple">
                                            <input type="text" name="fk_page_id" placeholder="Enter the Current URL"
                                                class="form-control" value="{{ $frontmenu->custom_url }}" />
                                        </div>
                                    @else
                            ...
                            <div class="form-group">
                                ...
                                <div class="select2-purple">
                                    <input type="text" name="title_hi" class="form-control" id="title_hi"
                                        ...
                                        value="{{ old('title_hi', $frontmenu->title_hi) }}" style="width: 100%;" />
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="open_same_tab" id="open_same_tab" class="form-control">
                                    <option value="0"
                                        {{ old('open_same_tab', $frontmenu->open_same_tab) == 0 ? 'selected' : '' }}>
                                        Same Tab</option>
                                    <option value="1"
                                        {{ old('open_same_tab', $frontmenu->open_same_tab) == 1 ? 'selected' : '' }}>
                                        New Tab</option>
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="menu_type" id="menu_type" class="form-control">
                                    <option value="1"
                                        {{ old('menu_type', $frontmenu->menu_type) == 1 ? 'selected' : '' }}>Page
                                    </option>
                                    <option value="2"
                                        {{ old('menu_type', $frontmenu->menu_type) == 2 ? 'selected' : '' }}>Module
                                    </option>
                                    <option value="3"
                                        {{ old('menu_type', $frontmenu->menu_type) == 3 ? 'selected' : '' }}>Custom URL
                                    </option>
                                    <option value="4"
                                        {{ old('menu_type', $frontmenu->menu_type) == 4 ? 'selected' : '' }}>Hash
                                    </option>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="select2-purple">
                                    <input type="text" name="icon_class" class="form-control" id="icon_class"
                                        ...
                                        value="{{ old('icon_class', $frontmenu->icon_class) }}"
                                        style="width: 100%;" />
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="select2-purple">
                                    <input type="text" name="title_en" class="form-control" id="title_en"
                                        placeholder="Enter Title in English"
                                        value="{{ old('title_en', $frontmenu->title_en) }}" style="width: 100%;" />
                                </div>
                                <x-input-error name="title_en" />
                            </div>
                            <x-admin.status-dropdown :selected="$frontmenu->status" />
                        </div>
                    </div>
                </div>
                <!-- /.card-body -->
                <div class="card-footer">
                    <x-admin.form-actions :actions="[
                        'update' => true,
                        'update_back' => route('manage.frontmenus.edit', $frontmenu->id),
                        'clear' => true,
                        'back' => route('manage.frontmenus.index', $type),
                    ]" />
                </div>
            </form>
        </x-admin.container-card>
