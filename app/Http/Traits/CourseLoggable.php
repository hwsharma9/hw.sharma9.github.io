...
trait CourseLoggable
{
    public static function createLog($old_model, Model $model)
    {
        ...
        $model->logs()->create([
            ...
            'fk_course_id' => self::getCourseId($model),
        ]);
    }

    public static function getCourseId(Model $model)
    {
        $fk_course_id = '';
        if ($model instanceof Course) {
            $fk_course_id = $model->id;
        } elseif ($model instanceof CourseTopic) {
            $fk_course_id = $model->fk_course_id;
    ...
    public static function bootCourseLoggable()
    {
    ...
    {
        return $this->morphMany(MCourseLog::class, 'loggable');
    }
